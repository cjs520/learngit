...
{
    $uid=(int)$uid;
    $rtl=$db->get_one("SELECT pic FROM `{$tablepre}want_buy` WHERE uid='$uid' AND m_uid='$m_check_uid' LIMIT 1");
    if($rtl)
    {
...
{
    $uid=(int)$uid;
    $rtl=$db->get_one("SELECT uid,approval_date FROM `{$tablepre}want_buy` WHERE uid='$uid' AND m_uid='$m_check_uid' LIMIT 1");
    if(!$rtl) exit('ERR:检索不到指定的求购信息');
    if($rtl['approval_date']<=10) exit('ERR:指定求购信息无法结束');
    
    $db->query("UPDATE `{$tablepre}want_buy` SET approval_date='10' WHERE uid='$rtl[uid]'");
    $db->free_result();
    exit('OK:设置成功');
}
else if($cmd=='add')
{
    if($_POST && (int)$step==1)
    {
        ...
        $db->insert("`{$tablepre}want_buy`",$row);
        $db->free_result();
        show_msg('求购信息添加成功，请等待管理员审核',"member.php?action=$action");
    }
    
    $pic='images/noimages/noproduct.jpg';
    require 'header.php';
    include template('member_want_buy_add');
}
else if($cmd=='edit')
{
    $uid=(int)$uid;
    $supply=$db->get_one("SELECT * FROM `{$tablepre}want_buy` WHERE uid='$uid' AND m_uid='$m_check_uid' LIMIT 1");
...
else
{
    ...
    require 'header.php';
    include template('member_want_buy');
}
