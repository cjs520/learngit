...
if($cmd=='upload')
{
    ...
    if(!in_array(dirname($_SERVER['PHP_SELF']),array('\\','/'))) $php_url=dirname($_SERVER['PHP_SELF']).'/';
    else $php_url = '/';

    //文件保存目录路径
    $save_path = $php_path . 'union/shopimg/user_img/'.$m_check_uid.'/';
    if(!is_dir($save_path)) mkdir($save_path,0777,true);
    //文件保存目录URL
    $save_url = $php_url . 'union/shopimg/user_img/'.$m_check_uid.'/';
    //定义允许上传的文件扩展名
    $ext_arr = array(
    ...
    if (empty($_FILES) === false)
    {
        ...
        if (!copy($tmp_name, $file_path)) alert("上传文件失败。");
        if($ucfg['mm_big_wate'])
        {
            require_once 'include/watermark.class.php';
            $file_path=Watermark::SetWaterMark($file_path,$ucfg,true);
...
else if($cmd=='list_files')
{
    ...
    $root_path = $php_path . 'union/shopimg/user_img/'.$m_check_uid.'/';
    if(!is_dir($root_path)) mkdir($root_path,0777,true);
    //根目录URL，可以指定绝对路径，比如 http://www.yoursite.com/attached/
    $root_url = $php_url . 'union/shopimg/user_img/'.$m_check_uid.'/';
    //图片扩展名
    ...
    //目录不存在或不是目录
    if (!file_exists($current_path) || !is_dir($current_path)) exit('Directory does not exist.');
