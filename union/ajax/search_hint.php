...
if($ctrl=='g' && (int)$sellshow==1)    //销售商品
{
    ...
    {
        $arr_rtl[]=array($rtl['goods_name'],GetBaseUrl('product',$rtl['uid'],'action','1',$rtl['supplier_id']));
    }
    $db->free_result();
}
...
else if($ctrl=='article')
{
    ...
    while ($rtl=$db->fetch_array($q))
    {
        $arr_rtl[]=array($rtl['article_name'],"$main_settings[mm_mall_url]/article.php?action=$rtl[ps_name]&id=$rtl[uid]");
    }
    $db->free_result();
}
else if($ctrl=='shop')
