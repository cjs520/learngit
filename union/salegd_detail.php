...
$uid=(int)$action;
$goods_table="{$tablepre}goods_onsale";
$product=$db->get_one("SELECT uid,goods_name,goods_sale_price,goods_code,goods_file1,goods_brand,goods_status,goods_stock,filter_attr,type,start_date,end_date 
                       FROM `$goods_table` 
                       WHERE uid='$uid' AND supplier_id='$page_member_id' 
                       LIMIT 1");
if(!$product) show_msg('检索不到指定的商品');

$detail_table="{$tablepre}goods_onsale_detail";
$detail=$db->get_one("SELECT goods_key,goods_market_price,goods_file2,attr_val,attr_store 
                      FROM `{$tablepre}goods_onsale_detail` 
                      WHERE g_uid='$uid' 
                      LIMIT 1");
$product=array_merge($product,$detail);
unset($detail);

$db->query("UPDATE `{$tablepre}goods_onsale` SET goods_hit=goods_hit+1 WHERE uid='$uid'");
$db->free_result();
...
//已抢数量
$rtl=$db->get_one("SELECT SUM(buy_number) AS sm FROM `{$tablepre}order_goods` WHERE goods_table='$goods_table' AND g_uid='$product[uid]' AND status='1'");
$product['buy_num']=(int)$rtl['sm'];
...
$q=$db->query("SELECT thumb,imgbig FROM `{$tablepre}onsale_gallery` WHERE goods_id='$product[uid]' LIMIT 5");
...
//qrcode
$qrcode_img=create_qrcode(md5($product['goods_name']),GetBaseUrl('salegd_detail',$product['uid'],'action','1',$page_member_id));
...
if($m_check_uid)
{
    ...
    $q=$db->query("SELECT uid,goods_name,goods_file1,supplier_id FROM `{$tablepre}goods_onsale` WHERE uid IN ($str_g_uid) LIMIT 6");
    while ($rtl=$db->fetch_array($q))
    {
        if(!file_exists($rtl['goods_file1'])) $rtl['goods_file1']='images/noimages/noproduct.jpg';
        $rtl['url']=GetBaseUrl('salegd_detail',$rtl['uid'],'action','1',$rtl['supplier_id']);
        $arr_history[]=$rtl;
    }
    $db->free_result();
...
{
    $q=$db->query("SELECT uid,goods_name,goods_file1 FROM `{$tablepre}goods_onsale` WHERE supplier_id='$page_member_id' ORDER BY start_date DESC LIMIT 6");
    while ($rtl=$db->fetch_array($q))
    {
        if(!file_exists($rtl['goods_file1'])) $rtl['goods_file1']='images/noimages/noproduct.jpg';
        $rtl['url']=GetBaseUrl('salegd_detail',$rtl['uid']);
        $arr_history[]=$rtl;
    }
    $db->free_result();
}
...
if(!$arr_relation)
{
    $q=$db->query("SELECT uid,goods_file1,goods_name,goods_sale_price FROM `$goods_table` WHERE supplier_id='$page_member_id' ORDER BY start_date DESC LIMIT 6");
    while ($rtl=$db->fetch_array($q))
    {
...
include_once template('product_onsale');
footer();