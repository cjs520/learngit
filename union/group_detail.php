 ...
 * $Date: 2008-06-25 $
 * $Id: group_detail.php  www.mvmmall.cn$
 * ---------------------------------------------
*/
require_once 'include/common.inc.php';
require_once 'header.php';

$goods_table="{$tablepre}goods_group";
$detail_table="{$tablepre}goods_group_detail";
$uid=(int)$action;
...
$product['discount']=round($product['goods_sale_price']/$product['goods_market_price']*10,2);
...
$q=$db->query("SELECT thumb,imgbig FROM `{$tablepre}group_gallery` WHERE goods_id='$product[uid]' LIMIT 5");
...
//qrcode
$qrcode_img=create_qrcode(md5($product['goods_name']),GetBaseUrl('group_detail',$product['uid'],'action','1',$page_member_id));
...
if($m_check_uid)
{
    ...
    while ($rtl=$db->fetch_array($q))
    {
        ...
        $rtl['url']=GetBaseUrl('group_detail',$rtl['uid'],'action','1',$rtl['supplier_id']);
        $arr_history[]=$rtl;
    }
    $db->free_result();
...
{
    $q=$db->query("SELECT uid,goods_name,goods_file1 
                   FROM `{$goods_table}` 
    ...
    {
        if(!file_exists($rtl['goods_file1'])) $rtl['goods_file1']='images/noimages/noproduct.jpg';
        $rtl['url']=GetBaseUrl('group_detail',$rtl['uid']);
        $arr_history[]=$rtl;
    }
    $db->free_result();
}
...
include_once template('product_group');
footer();