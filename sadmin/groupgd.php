...
if($action=='list')
{
    ...
    $q = $db->query("SELECT uid,goods_name,goods_sale_price,goods_file1,supplier_id,start_date,end_date,approval,reject 
                     FROM `{$tablepre}goods_group` 
                     $search_sql $order_sql 
                     LIMIT $from_record, $list_num");
    while ($rt = $db->fetch_array($q))
    {
        $rt['url'] = GetBaseUrl('group_detail',$rt['uid'],'action',$rt['supplier_id']);
        ...
        $rt['rehandout']=$rt['approval']>0 && $rt['end_date']<=$m_now_time;
	...
	include template('sadmin_goods_group');
}
else if($action=='add' || $action=='edit')
{
	if($action=='add')    //未审核时查看一下商品
	{
		if($shop_file['isSupplier']<=2) show_msg('您的商铺还未通过审核，无法申请团购活动');
		if($mvm_member['member_point']<floatval($mm_groupgd_point)) show_msg('您的积分不足以发布团购活动，请先充值');
	}
    ...
    if ($_POST && (int)$step==1)
    {
        ...
        if($action=='edit')
        {
            $product=$db->get_one("SELECT goods_file1,approval FROM `{$tablepre}goods_group` WHERE uid='$uid' AND supplier_id='$page_member_id' LIMIT 1");
            if(!$product) show_msg('检索不到指定的商品');
            if($product['approval']>0) show_msg('本团购活动已通过审核，无法修改');
            $tmp_rtl=$db->get_one("SELECT goods_file2 FROM `{$tablepre}goods_group_detail` WHERE g_uid='$uid' LIMIT 1");
        ...
        if ($action=='add')
        {
        	$goods_id = $db->insert("{$tablepre}goods_group",$row);
        	$detail_row['g_uid']=$goods_id;
        	$db->insert("`{$tablepre}goods_group_detail`",$detail_row);
        	add_score($m_check_uid,floatval($mm_groupgd_point)*-1,'团购申请',$row['goods_name'].' 团购申请');
        }
        else if($action=='edit' && is_numeric($uid))
        {
        	$goods_id = $uid = (int)$uid;
            $db->update("{$tablepre}goods_group",$row,$where="uid='$uid'");
            $detail_row['g_uid']=$goods_id;
        	$db->replace("`{$tablepre}goods_group_detail`",$detail_row);
        }
        ...
        {
            $rtl=$db->get_one("SELECT COUNT(*) AS cnt FROM `{$tablepre}group_gallery` WHERE goods_id='$goods_id'");
        	...
        	foreach ($_FILES['gallery']['name'] as $key => $val)
            {
        	    ...
        	    $db->query("INSERT INTO `{$tablepre}group_gallery` SET 
        	                goods_id='$goods_id',
        	                imgbig='$imgbig',
        	                thumb='$thumb',
        	                supplier_id='$page_member_id'");
    ...
    {
        //商品信息
    	$product = $db->get_one("SELECT * FROM `{$tablepre}goods_group` WHERE uid='$uid' AND `supplier_id`='$page_member_id' LIMIT 1");
    	if($product)
    	{
    	    $product_detail=$db->get_one("SELECT * FROM `{$tablepre}goods_group_detail` WHERE g_uid='$uid' LIMIT 1");
    	...
    	//相册
        $photo = $db->get_all("SELECT imgid,thumb FROM `{$tablepre}group_gallery` 
	...
	include template('sadmin_goods_group_add');
}
else if($action=='del')
{
 	if (is_numeric($uid))
...
else if($action=='bat_move')
{
	if(is_array($uid_check) && $uid_check)
    {
    	...
    	if($uid_check && $rtl)
    	{
    	     $db->query("UPDATE `{$tablepre}goods_group` SET supplier_cat='$cat_menu_move' WHERE uid IN ($str_uid) AND `supplier_id`='$page_member_id'");
    	}
    	$db->free_result();
    }
	exit('OK:成功更新商品分类');
...
else if($action=='ajax_edit')
{
	...
	{
		$db->query("UPDATE `{$tablepre}goods_group` SET `$f`='$v' WHERE uid='$uid' AND supplier_id='$page_member_id'");
	}
	exit('修改成功');
}
else if($action=='delimg')    //ajax删除单个相册图片
{
	$uid=(int)$uid;
	if($uid<=0) exit;
    $gallery = $db->get_one("SELECT * FROM `{$tablepre}group_gallery` WHERE imgid='$uid' AND supplier_id='$page_member_id' LIMIT 1");
    if(!$gallery) exit;
    file_unlink(ProcImgPath($gallery['imgbig']));
    file_unlink(ProcImgPath($gallery['thumb']));
    $db->query("DELETE FROM `{$tablepre}group_gallery` WHERE imgid='$uid'"); 
    exit;
}
...
{
    if($mvm_member['member_point']<floatval($mm_groupgd_point)) exit('ERROR:您的积分不足以发布团购活动，请先充值');
    $uid=(int)$uid;
    $g=$db->get_one("SELECT * FROM `{$tablepre}goods_group` WHERE uid='$uid' AND supplier_id='$page_member_id' LIMIT 1");
    if(!$g) exit('ERROR:检索不到指定的团购活动');
    $g['rel_uid']=$g['uid'];
    $g['approval']=0;
    $g['reject']='';
    $g['start_date']=$g['end_date']=$m_now_time;
    unset($g['uid']);
    $insert_id=$db->insert("`{$tablepre}goods_group`",$g);
    $db->free_result();
    
    $detail=$db->get_one("SELECT * FROM `{$tablepre}goods_group_detail` WHERE g_uid='$uid' LIMIT 1");
    if($detail)
    {
        $detail['g_uid']=$insert_id;
        $db->replace("`{$tablepre}goods_group_detail`",$detail);
    }
    $db->free_result();
    
    $q=$db->query("SELECT * FROM `{$tablepre}group_gallery` WHERE goods_id='$uid'");
    while ($rtl=$db->fetch_array($q))
    {
        $rtl['goods_id']=$insert_id;
        unset($rtl['imgid']);
        $db->insert("`{$tablepre}group_gallery`",$rtl);
    }
    $db->free_result();
    
    add_score($m_check_uid,floatval($mm_groupgd_point)*-1,'团购申请',"团购申请 $g[goods_name]");
    exit('OK:团购活动重发成功');
}
...
function del_goods($uid)
{
    ...
    $rtl = $db->get_one("SELECT uid,goods_file1 FROM `{$tablepre}goods_group` WHERE uid = '$uid' AND `supplier_id`='$page_member_id' LIMIT 1");
    if(!$rtl) exit;
    $detail=$db->get_one("SELECT goods_file2 FROM `{$tablepre}goods_group_detail` WHERE g_uid='$uid'");
    if($detail) $rtl=array_merge($rtl,$detail);
    
    $rel_rtl=$db->get_one("SELECT uid FROM `{$tablepre}goods_group` WHERE supplier_id='$page_member_id' AND rel_uid='$uid' AND uid<>'$uid' LIMIT 1");
    ...
    $db->query("DELETE FROM `{$tablepre}group_gallery` WHERE goods_id='$uid'");
    $db->query("DELETE FROM `{$tablepre}goods_group_detail` WHERE g_uid='$uid'");
    $db->query("DELETE FROM `{$tablepre}goods_group` WHERE uid='$uid'");
    $db->free_result();
}