...
if($action=='list')
{
	...
	include template('sadmin_downgoods');
}
else if($action=='add' || $action=='edit')
{
    ...
    if ($_POST && (int)$step==1)
    {
        ...
        if($action=='edit')
        {
            $product=$db->get_one("SELECT goods_file1 FROM `{$tablepre}downgoods_table` WHERE uid='$uid' AND supplier_id='$page_member_id' LIMIT 1");
    ...
    {
        //商品信息
    	$product = $db->get_one("SELECT * FROM `{$tablepre}downgoods_table` WHERE uid='$uid' AND `supplier_id`='$page_member_id' LIMIT 1");
    	if($product)
    	{
	...
	include template('sadmin_downgoods_add');
}
else if($action=='del')
{
 	if (is_numeric($uid))
...
else if($action=='bat_up')
{
    if(is_array($uid_check) && $uid_check)
    {
    	foreach ($uid_check as $key=>$val)
    	{
    		...
    		$rtl=$db->get_one("SELECT * FROM `{$tablepre}downgoods_table` WHERE uid='$val' AND supplier_id='$page_member_id' LIMIT 1");
    		if(!$rtl) continue;
...
else if($action=='ajax_edit')
{
	...
	{
		$db->query("UPDATE `{$tablepre}downgoods_table` SET `$f`='$v' WHERE uid='$uid' AND supplier_id='$page_member_id'");
	}
	exit('修改成功');
}
else if($action=='delimg')    //ajax删除单个相册图片
{
	$uid=(int)$uid;
	if($uid<=0) exit;
    $gallery = $db->get_one("SELECT * FROM `{$tablepre}gallery` WHERE imgid='$uid' AND supplier_id='$page_member_id' LIMIT 1");
    if(!$gallery) exit;
...
else if($action=='search_combine')    //搜索商品，用于组合购买
{
    ...
    $q=$db->query("SELECT uid,goods_file1,goods_name,goods_sale_price 
                   FROM `{$tablepre}goods_table` WHERE goods_name LIKE '%$key%' AND supplier_id='$page_member_id' AND type IN (0,1,3) AND uid<>'$uid' 
                   LIMIT 10");
    while ($rtl=$db->fetch_array($q))
    {
...
function del_goods($uid)
{
    ...
    $rtl = $db->get_one("SELECT uid,goods_file1 FROM `{$tablepre}downgoods_table` WHERE uid = '$uid' AND `supplier_id`='$page_member_id' LIMIT 1");
    if(!$rtl) exit;
