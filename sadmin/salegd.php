...
if($action=='list')
{
    ...
    $q = $db->query("SELECT uid,goods_name,goods_sale_price,goods_file1,supplier_id,start_date,end_date 
                     FROM `{$tablepre}goods_onsale` 
                     $search_sql $order_sql 
                     LIMIT $from_record, $list_num");
    while ($rt = $db->fetch_array($q))
    {
        $rt['url'] = GetBaseUrl('salegd_detail',$rt['uid'],'action',$rt['supplier_id']);
	...
	include template('sadmin_goods_onsale');
}
else if($action=='add' || $action=='edit')
{
	if($action=='add')    //未审核时查看一下商品
	{
		if($shop_file['isSupplier']<=2) show_msg('您的商铺还未通过审核，无法发布促销商品');
	}
    ...
    if ($_POST && (int)$step==1)
    {
        ...
        if($action=='edit')
        {
            $product=$db->get_one("SELECT goods_file1 FROM `{$tablepre}goods_onsale` WHERE uid='$uid' AND supplier_id='$page_member_id' LIMIT 1");
            if(!$product) show_msg('检索不到指定的商品');
            $tmp_rtl=$db->get_one("SELECT goods_file2 FROM `{$tablepre}goods_onsale_detail` WHERE g_uid='$uid' LIMIT 1");
        ...
        if ($action=='add')
        {
        	$goods_id = $db->insert("{$tablepre}goods_onsale",$row);
        	$detail_row['g_uid']=$goods_id;
        	$db->insert("`{$tablepre}goods_onsale_detail`",$detail_row);
        }
        else if($action=='edit' && is_numeric($uid))
        {
        	$goods_id = $uid = (int)$uid;
            $db->update("{$tablepre}goods_onsale",$row,$where="uid='$uid'");
            $detail_row['g_uid']=$goods_id;
        	$db->replace("`{$tablepre}goods_onsale_detail`",$detail_row);
        }
        ...
        {
            $rtl=$db->get_one("SELECT COUNT(*) AS cnt FROM `{$tablepre}onsale_gallery` WHERE goods_id='$goods_id'");
        	...
        	foreach ($_FILES['gallery']['name'] as $key => $val)
            {
        	    ...
        	    $db->query("INSERT INTO `{$tablepre}onsale_gallery` SET 
        	                goods_id='$goods_id',
        	                imgbig='$imgbig',
        	                thumb='$thumb',
        	                supplier_id='$page_member_id'");
    ...
    {
        //商品信息
    	$product = $db->get_one("SELECT * FROM `{$tablepre}goods_onsale` WHERE uid='$uid' AND `supplier_id`='$page_member_id' LIMIT 1");
    	if($product)
    	{
    	    $product_detail=$db->get_one("SELECT * FROM `{$tablepre}goods_onsale_detail` WHERE g_uid='$uid' LIMIT 1");
    	...
    	//相册
        $photo = $db->get_all("SELECT imgid,thumb FROM `{$tablepre}onsale_gallery` 
	...
	include template('sadmin_goods_onsale_add');
}
else if($action=='del')
{
 	if (is_numeric($uid))
...
else if($action=='bat_move')
{
	if(is_array($uid_check) && $uid_check)
    {
    	...
    	if($uid_check && $rtl)
    	{
    	     $db->query("UPDATE `{$tablepre}goods_onsale` SET supplier_cat='$cat_menu_move' WHERE uid IN ($str_uid) AND `supplier_id`='$page_member_id'");
    	}
    	$db->free_result();
    }
	exit('OK:成功更新商品分类');
...
else if($action=='ajax_edit')
{
	...
	{
		$db->query("UPDATE `{$tablepre}goods_onsale` SET `$f`='$v' WHERE uid='$uid' AND supplier_id='$page_member_id'");
	}
	exit('修改成功');
}
else if($action=='delimg')    //ajax删除单个相册图片
{
	$uid=(int)$uid;
	if($uid<=0) exit;
    $gallery = $db->get_one("SELECT * FROM `{$tablepre}onsale_gallery` WHERE imgid='$uid' AND supplier_id='$page_member_id' LIMIT 1");
    if(!$gallery) exit;
    file_unlink(ProcImgPath($gallery['imgbig']));
    file_unlink(ProcImgPath($gallery['thumb']));
    $db->query("DELETE FROM `{$tablepre}onsale_gallery` WHERE imgid='$uid'"); 
    exit;
}
...
function del_goods($uid)
{
    ...
    $rtl = $db->get_one("SELECT uid,goods_file1 FROM `{$tablepre}goods_onsale` WHERE uid = '$uid' AND `supplier_id`='$page_member_id' LIMIT 1");
    if(!$rtl) exit;
    $detail=$db->get_one("SELECT goods_file2 FROM `{$tablepre}goods_onsale_detail` WHERE g_uid='$uid'");
    if($detail) $rtl=array_merge($rtl,$detail);
    
    file_unlink(ProcImgPath($rtl['goods_file1']));
    file_unlink(ProcImgPath($rtl['goods_file2']));
    $q=$db->query("SELECT * FROM `{$tablepre}onsale_gallery` WHERE goods_id='$uid'");
    while($gallery=$db->fetch_array($q))
    {
        file_unlink(ProcImgPath($gallery['imgbig']));
        file_unlink(ProcImgPath($gallery['thumb']));
    }
    $db->query("DELETE FROM `{$tablepre}onsale_gallery` WHERE goods_id='$uid'");
    $db->query("DELETE FROM `{$tablepre}goods_onsale_detail` WHERE g_uid='$uid'");
    $db->query("DELETE FROM `{$tablepre}goods_onsale` WHERE uid='$uid'");
    $db->free_result();
}