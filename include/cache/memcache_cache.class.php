...
class memcache_cache
{
    ...
    public function get_cache($cache_id,$userid=0)
    {
        ...
        else $memcache_key=$this->cache_settings['memcache_pre'].$cache_id;
        //防并发读取
        $tick=0;
        $data=null;
        do
    ...
    public function put_cache($cache_id,$userid=0)
    {
        ...
        {
            $this->mem->set($this->cache_settings['memcache_pre'].$cache_id,$cache_data);
            $this->mem->set($this->cache_settings['memcache_pre'].$cache_id.'_expire',$this->expire);
        }
        return $cache_data;
    }//end put_cache
    ...
    public function delete($cache_name,$supplier_id)
    {
        ...
        if($supplier_id>0)
        {
            $this->mem->delete($this->cache_settings['memcache_pre']."user_cache/$supplier_id/".$cache_name);
            $this->mem->delete($this->cache_settings['memcache_pre']."user_cache/$supplier_id/".$cache_name.'_expire');
        }
        else
        {
            $this->mem->delete($this->cache_settings['memcache_pre'].$cache_name);
            $this->mem->delete($this->cache_settings['memcache_pre'].$cache_name.'_expire');
            $this->mem->delete($this->cache_settings['memcache_pre'].'union/'.$cache_name);
            $this->mem->delete($this->cache_settings['memcache_pre'].'union/'.$cache_name.'_expire');
        }
    }//end delete
    ...
    {
        $memcache_key=$this->cache_settings['memcache_pre'].md5($cache_file);
        
        $this->mem->set($memcache_key,$cache_data);
        $this->mem->set($memcache_key.'_expire',$this->expire);
        return $cache_data;
    }//end write_cache
