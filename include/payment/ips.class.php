...
$payment['ips']['reg'] = 'http://main.ips.com.cn/';    //申请地址
$payment['ips']['license'] = '<a href="http://main.ips.com.cn/" target="_blank">免费签约</a>';    //版权信息
//接口需要的参数
...
class ips
{
    ...
    function pay_send($sn,$amount)    //提交支付请求
	{
		...
		$Merchanturl="$mm_mall_url/respond.php?sessionID=$sessionID&code=".basename(__FILE__, '.class.php');    //商户返回地址
    ...
    function pay_receive()    //提交返回处理
    {
        ...
        if ($signature_1ocal != $signature) show_msg('支付安全性校验失败','./');
        $sn=$billno;
        
        require_once 'include/order.class.php';
        $salt=substr($sn,strlen($sn)-4,4);
        $sn=substr($sn,0,strlen($sn)-4);
        if(order::check_pay_log($sn,$salt,$amount)) add_money($m_check_uid,$amount,'预付款充值','环迅支付'.$sn.'充值',$sn);
