...
class tenpay
{
    ...
    function pay_send($sn,$amount)	
	{
        ...
        $return_url = "{$mm_mall_url}/respond.php?sessionID=$sessionID&code=".basename(__FILE__, '.class.php');
        $strDate = date("Ymd");
        $strTime = date("His");
		...
		$result='<a href="'.trim($result).'" rel="pay_a" target="_blank"><img src="images/pay/tenpay.gif" /></a>';
		return $result;
	}
    ...
    function pay_receive()
    {
	    ...
	    //判断签名
	    if(!$resHandler->isTenpaySign()) show_msg('安全校验失败','./');
	   
	    $sn = $resHandler->getParameter("sp_billno");    //商户订单号
	    $amount = $resHandler->getParameter("total_fee");    //金额,以分为单位
	    $pay_result = $resHandler->getParameter("pay_result");    //支付结果

	    if( "0" != $pay_result ) show_msg('订单返回结果错误','./');
        require_once 'include/order.class.php';
        $salt=substr($sn,strlen($sn)-4,4);
        $sn=substr($sn,0,strlen($sn)-4);
        $amount=round($amount/100,2);
        
        if(order::check_pay_log($sn,$salt,$amount)) add_money($m_check_uid,$amount,'预付款充值','财付通支付'.$sn.'充值',$sn);
