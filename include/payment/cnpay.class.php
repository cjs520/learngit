...
class cnpay
{
    ...
    function pay_send($sn,$amount)	
	{
	    ...
	    $c_returl = "$mm_mall_url/respond.php?sessionID=$sessionID";    //为空表示云网的;
	    $c_memo1 = '';    //备注1
	    $c_memo2 = '';    //备注2
    ...
    function pay_receive()    //提交返回处理
    {
		...
		if($r_signstr!=$c_signstr) show_msg('信息安全性校验失败','./');
		
		//--校验返回的支付结果的格式是否正确
		if($c_succmark != 'Y') show_msg('订单返回状态错误','./');
		$sn = dhtmlchars($c_order);
		require_once 'include/order.class.php';
        $salt=substr($sn,strlen($sn)-4,4);
        $sn=substr($sn,0,strlen($sn)-4);
        if(order::check_pay_log($sn,$salt,$amount)) add_money($m_check_uid,$amount,'预付款充值','云网在线支付'.$sn.'充值',$sn);
