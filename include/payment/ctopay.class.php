...
class ctopay
{
    ...
    function pay_send($sn,$amount)	
	{
        ...
        $Language = '1';    //语言
        $ReturnURL = "$mm_mall_url/respond.php?sessionID=$sessionID&code=".basename(__FILE__, '.class.php');    //返回地址
        $Remark = '';    //备注
    ...
    function pay_receive()    //提交返回处理
    {
        ...
        if ($MD5info!=$md5sign) show_msg('支付安全较验失败','./');
        if ($Succeed != '1') show_msg('订单返回状态错误','./');
        
        $sn= strip_tags($BillNo);
        require_once 'include/order.class.php';
        $salt=substr($sn,strlen($sn)-4,4);
        $sn=substr($sn,0,strlen($sn)-4);
        if(order::check_pay_log($sn,$salt,$amount)) add_money($m_check_uid,$amount,'预付款充值','收汇宝支付'.$sn.'充值',$sn);
