 ...
 * $Date: 2008-02-27 $
 * $Id: mysql_class.php  www.mvmmall.cn$
 * ---------------------------------------------
*/
...
class dbmysql
{
	...
	{
		!($this->link = @mysql_connect($con_db_host,$con_db_id,$con_db_pass, 1)) && $this->halt('Can not connect to MySQL server');
		
		mysql_query("SET sql_mode=''", $this->link);
		mysql_query("SET NAMES $db_charset",$this->link);
		mysql_query("USE $con_db_name", $this->link);
	}
	...
	{
		if(!($query = mysql_query($sql, $this->link))) $this->halt('MySQL Query Error', $sql);
	...
	{
	    $insert_id=mysql_result($this->query("SELECT last_insert_id()"), 0);
	    $this->free_result(1);
		return $insert_id;
	}
