 ...
 * $Date: 2008-06-28 $
 * $Id: infor_supply_more.php  www.mvmmall.cn$
 * ---------------------------------------------
*/
...
$q = $db->query("SELECT uid,goods_name,intro,province,city,county,register_date FROM `{$tablepre}want_supply` 
	             $filter_sql 
...
while($rtl = $db->fetch_array($q))
{
    ...
    $rtl['url']=GetBaseUrl('infor_supply_detail',$rtl['uid']);
    if($search_txt) $rtl['goods_name']=str_replace($search_txt,"<b class='red'>$search_txt</b>",$rtl['goods_name']);
    $arr_info[]=$rtl;
}
$db->free_result();

$page_list = $rowset->link("infor_supply_more.php?cat_uid=$cat_uid&search_txt=".urlencode($search_txt)."&page=");
...
include template('infor_supply_more');
footer();

function get_infor_list($cat_uid,$is_top=0,$limit=5)
{
    global $cache;
    $arr_infor_supply=$cache->read_cache("data/cache/supply_info_{$cat_uid}_{$is_top}.cache.php",
                                         'get_infor_list_from_db',
                                         array('cat_uid'=>$cat_uid,'is_top'=>$is_top,'limit'=>$limit),
                                         'arr_infor_supply');
    return $arr_infor_supply;
}
