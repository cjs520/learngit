...
else
{
    ...
    if(!$pay) show_msg('pass_worng','close');
    else require_once 'include/payment/'. $pay['class_name'].'.class.php';
...
function order_prepare($sn)    //检测是否已经支付过
{
    ...
    if(!$m_check_uid) show_msg('您还未登录，请先登录',GetBaseUrl('logging','login'));
	...
	switch ($ms_flag)
	{
	    case 'up':    //商铺升级订单
	        ...
	        if(!$order) show_msg('检索不到指定的订单');
	        if($order['approval_date']>10) show_msg('指定的升级记录已支付',"sadmin.php?module=shop_update&action=list");
	        break;
	    case 'od':    //普通购物订单
	    case 'au':    //拍卖订单
	        $order=$db->get_one("SELECT status,goods_rest_amount FROM `{$tablepre}order_info` WHERE ordersn='$sn' LIMIT 1");
	        if(!$order) show_msg('检索不到指定的订单','./');
	        if($order['status']!=1 && $order['goods_rest_amount']<=0) show_msg('指定的订单已付款','member.php?action=index');
	        break;
	    case 'ct':    //普通购物订单组合购买
	        $order=$db->get_one("SELECT approval_date FROM `{$tablepre}order_combine` WHERE tag='$sn' LIMIT 1");
	        if(!$order) show_msg('检索不到指定的订单','./');
	        if($order['approval_date']>10) show_msg('指定的订单已付款','member.php?action=index');
	        break;
	}
...
function change_order($sn)
{
	...
	if($ms_flag=='up')    //商铺升级
	{
	    ...
	    if($m['member_money']<$order['amount']) show_msg('您的预付款余额不足，请充值');
	    ...
	    if((int)$GLOBALS['mm_mail_shop_update']==1 && $m['member_email'])
	    {
	        smtp_mail($m['member_email'],
	                  str_replace(array('{shop_name}'),array($shop['shop_name']),'您的商铺{shop_name}升级成功'),
	                  str_replace(array('{shop_name}','{shop_level}'),array($shop['shop_name'],$lang['shop_level'][$order['shop_level']]),'您的商铺{shop_name}已经升级为{shop_level}，恭喜')
	        );
	    }
	...
	if($ms_flag=='rn')    //商铺续费
	{
	    ...
	    if(!$pay_log) show_msg('检索不到指定的支付记录');
	    $db->query("DELETE FROM `{$tablepre}pay_log` WHERE tag='$sn'");
	    
	    $m=cur_member_info($m_check_uid);
	    if($m['member_money']<$pay_log['total_money']) show_msg('您的预付款余额不足，请充值');
	    
	    $arr_info=explode('|',$pay_log['other_info']);
	    if(sizeof($arr_info)!=3) show_msg('信息错误');
	    $shop_level=(int)$arr_info[1];
	    $year=(int)$arr_info[0];
	    $supplier_id=(int)$arr_info[2];
	    if($year<=0) show_msg('续费年限错误','sadmin.php?module=shop_renew&action=list');
	    if($shop_level<=0 || $shop_level>4) show_msg('续费等级错误','sadmin.php?module=shop_renew&action=list');
	    $shop=$db->get_one("SELECT shop_level,shop_expire FROM `{$tablepre}member_shop` WHERE m_uid='$supplier_id' LIMIT 1");
	    if(!$shop) show_msg('检索不到指定的商铺','sadmin.php?module=shop_renew&action=list');
	...
	if($ms_flag=='xb')    //申请消保
	{
	    ...
	    $m=cur_member_info($m_check_uid);
	    if($m['member_money']<$total_money) show_msg('您的余额不足以支付消保申请');
	...
	if($ms_flag=='bp')    //积分购买
	{
	    ...
	    $m=cur_member_info($m_check_uid);
	    if($m['member_money']<$pay_log['total_money']) show_msg('您的预付款账户余额不足','account.php?action=money');
	...
	if($ms_flag=='tp')    //模板购买
	{
	    $pay_log=$db->get_one("SELECT total_money,other_info FROM `{$tablepre}pay_log` WHERE tag='$sn' LIMIT 1");
	    if(!$pay_log) show_msg('检索不到指定的支付记录');
	    if(!$pay_log['other_info']) show_msg('支付模板信息出错，请联系管理员');
	    $db->query("DELETE FROM `{$tablepre}pay_log` WHERE tag='$sn'");
	...
	else if(in_array($ms_flag,array('od','ct','au')))
	{
        global $mm_buy_point;
	    /*************** 处理普通订单 ****************/
	    $arr_orders=array();
	    ...
	    {
	        $order_info=$db->get_one("SELECT uid,code,ordersn,status,goods_amount,goods_rest_amount,sh_price,discount,goods_point,mobile,supplier_id 
	                                  FROM `{$tablepre}order_info` WHERE ordersn='$sn' 
	                                  LIMIT 1");
	        if($order_info) $arr_orders[]=$order_info;
	    }
	    else
	    {
	    ...
	    if(!$arr_orders) show_msg('检索不到指定的订单数据','./');
	    $total_point=0;
	    $total_money=0;
	    $send_point=0;
	    $other_info='';
	    ...
	    if($m['member_money']<$total_money) show_msg('您的余额不足，请重新支付','member.php?action=order');
	    
	    if($ms_flag=='od') $other_info="订单号：$sn";
	    else $other_info='订单号：'.substr($other_info,1);
	    ...
	    if($ms_flag=='ct') $db->query("UPDATE `{$tablepre}order_combine` SET approval_date='$m_now_time' WHERE tag='$sn'");
	    $db->free_result();
	    ...
	    foreach ($arr_orders as $val)
	    {
	        ...
	        if(in_array($val['supplier_id'],$arr_supplier_id)) continue;
	        ...
	        //发送短信
	        if(((int)$ucfg['mm_sms_receipt']!=1 || !$val['mobile'])) continue;
	        $send_message=str_replace(array('{ordersn}','{orderpass}'),array($val['ordersn'],$val['code']),$ucfg['mm_sms_message2']);
	        supplier_sms_send($val['mobile'],$send_message,$val['supplier_id']);
	    }

	    do    //发邮件
	    {
	        if((int)$GLOBALS['mm_mail_receipt']!=1) break;
	        if(!$mvm_member['member_email']) break;
	        if(!$arr_ordersn) break;
	        
	        $ordersn=implode(',',$arr_ordersn);
	        smtp_mail($mvm_member['member_email'],
	            str_replace('{ordersn}',$ordersn,"订单{ordersn}已付款成功"),
	            str_replace(array('{mall_title}','{ordersn}'),array($GLOBALS['mm_mall_title'],$ordersn),$GLOBALS['mm_mail_receipt_cnt'])
	        );
	    }while (0);
	    
	    show_msg('支付成功，祝您购物愉快','member.php?action=order');
	}
	else show_msg('订单类型错误，请联系管理员');
}