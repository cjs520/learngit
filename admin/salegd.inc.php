 ...
 * $Date: 2008-02-27 $
 * $Id: salegd.inc.php www.mvmmall.cn$
 * ---------------------------------------------
*/
...
if($action=='list')
{
    ...
    $q = $db->query("SELECT uid,goods_name,goods_file1,goods_sale_price,goods_code,goods_stock,start_date,end_date,supplier_id 
                     FROM `{$tablepre}goods_onsale` $search_sql $order_sql LIMIT $from_record, $list_num");
    while ($rt = $db->fetch_array($q))
    {
        ...
        $rt['goods_url'] = GetBaseUrl('salegd_detail',$rt['uid'],'action',$rt['supplier_id']);
    ...
    $page_list = $rowset->link("admincp.php?module=$module&action=$action&ps_subject=$ps_subject&ps_code=$ps_code&od=$od&page=");
    require_once template('goods_onsale');
    footer();
}
else if($action=='edit')
{
	$goods_id=$uid=(int)$uid;
    $product = $db->get_one("SELECT * FROM `{$tablepre}goods_onsale` WHERE uid='$uid' LIMIT 1");
    ...
    $photo = $db->get_all("SELECT imgid,thumb FROM `{$tablepre}onsale_gallery` WHERE goods_id='$uid'");//相册数据
    ...
    require_once template('goods_onsale_add');
    footer();
}
else if ($action=='delimg')    //ajax删除单个相册图片
{
    ...
    {
        $uid=(int)$uid;
        $gallery = $db->get_one("SELECT * FROM `{$tablepre}onsale_gallery` WHERE imgid='$uid' LIMIT 1");
        if(!$gallery) break;
        $g=$db->get_one("SELECT goods_name FROM `{$tablepre}goods_onsale` WHERE uid='$gallery[goods_id]' LIMIT 1");
...
else if($action=='ajax_edit')
{
    ...
    {
        $db->query("UPDATE `{$tablepre}goods_onsale` SET `$field`='$v' WHERE uid='$uid'");
        $db->free_result();
    }
    exit;
}
...
function del_goods($uid)
{
    ...
    $rtl = $db->get_one("SELECT uid,goods_file1,goods_name FROM `{$tablepre}goods_onsale` WHERE uid = '$uid' LIMIT 1");
    if(!$rtl) exit;
    admin_log("删除促销商品：$rtl[goods_name]");
