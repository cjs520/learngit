 ...
 * $Date: 2008-07-14 $
 * $Id: wx_auto_reply.inc.php www.mvmmall.cn$
 * ---------------------------------------------
*/
...
if ($action=='list')
{
    ...
    $q=$db->query("SELECT uid,title,description,pic,url FROM `{$tablepre}wx_auto_reply` WHERE supplier_id=0 AND type='news' ORDER BY od LIMIT 5");
    ...
    require template('wx_auto_reply');
    footer();
}
else if($action=='add')
{
...
{
    $uid=(int)$uid;
    $rtl=$db->get_one("SELECT uid,title,pic FROM `{$tablepre}wx_auto_reply` WHERE uid='$uid' AND supplier_id='0' LIMIT 1");
    if(!$rtl) exit('ERR:检索不到您指定的记录');
    file_unlink($rtl['pic']);
    admin_log("删除微信自动回复：$rtl[title]");
