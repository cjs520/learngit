 ...
 * $Date: 2008-07-14 $
 * $Id: want_supply.inc.php www.mvmmall.cn$
 * ---------------------------------------------
*/
...
if($action=='list')
{
    ...
    $total_count = $db->counter("{$tablepre}want_supply",$search_sql);
    $page = $page ? (int)$page:1;
    $list_num = 10;
    $rowset = new Pager($total_count,$list_num,$page);
    $from_record = $rowset->_offset();
    $q = $db->query("SELECT uid,goods_name,goods_category,num,price,register_date,approval_date,pic,back_reason,od,m_uid FROM `{$tablepre}want_supply` 
                     $search_sql 
                     ORDER BY `uid` DESC  
    ...
    require_once template('want_supply');
    footer();
}
else if ($action=='edit' && is_numeric($uid))
{
	$uid = (int)$uid;
    $supply=$db->get_one("SELECT * FROM `{$tablepre}want_supply` WHERE uid='$uid' LIMIT 1");
    if(!$supply) sadmin_show_msg('检索不到指定的供应信息',$p_url);
    ...
    if($_POST && (int)$step==1)
    {
        ...
        admin_log("编辑供应信息：$supply[goods_name]");
        iframe_callback('','');
    }
    $pic=$supply['pic'];
    
    if(!$pic || !file_exists($pic)) $pic='images/noimages/noproduct.jpg';
    require_once template('want_supply_add');
    footer(false);
}
else if ($action=='del')
{
    $uid=(int)$uid;
    $rtl=$db->get_one("SELECT uid,goods_name,pic FROM `{$tablepre}want_supply` WHERE uid='$uid' LIMIT 1");
    if($rtl)
    {
        file_unlink($rtl['pic']);
        admin_log("删除供应信息：$rtl[goods_name]");
...
{
    $uid=(int)$uid;
    $rtl=$db->get_one("SELECT uid,goods_name,approval_date FROM `{$tablepre}want_supply` WHERE uid='$uid' LIMIT 1");
    if($rtl)
    {
        admin_log("审核供应信息：$rtl[goods_name]");
        $db->query("UPDATE `{$tablepre}want_supply` SET approval_date='$m_now_time' WHERE uid='$uid'");
        $db->free_result();
    }
...
else if($action=='back')
{
    ...
    if($rtl)
    {
        admin_log("驳回供应信息：$rtl[goods_name]");
        $db->query("UPDATE `{$tablepre}want_supply` SET 
                    approval_date='-1',
...
else if($action=='set_od')
{
    $uid=(int)$uid;
    $od=(int)$od;
    $db->query("UPDATE `{$tablepre}want_supply` SET od='$od' WHERE uid='$uid'");
    $db->free_result();
    exit;
}
