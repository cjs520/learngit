 ...
 * $Date: 2008-02-27 $
 * $Id: groupgd.inc.php www.mvmmall.cn$
 * ---------------------------------------------
*/
...
if($action=='list')
{
    ...
    $q = $db->query("SELECT uid,goods_name,goods_file1,goods_sale_price,goods_code,goods_stock,start_date,end_date,approval,reject,supplier_id 
                     FROM `{$tablepre}goods_group` $search_sql $order_sql LIMIT $from_record, $list_num");
    while ($rt = $db->fetch_array($q))
    {
        ...
        $rt['goods_url'] = GetBaseUrl('group_detail',$rt['uid'],'action',$rt['supplier_id']);
    ...
    require_once template('goods_group');
    footer();
}
else if($action=='edit')
{
	$goods_id=$uid=(int)$uid;
    $product = $db->get_one("SELECT * FROM `{$tablepre}goods_group` WHERE uid='$uid' LIMIT 1");
    ...
    $photo = $db->get_all("SELECT imgid,thumb FROM `{$tablepre}group_gallery` WHERE goods_id='$uid'");//相册数据
    ...
    require_once template('goods_group_add');
    footer();
}
else if ($action=='delimg')    //ajax删除单个相册图片
{
    ...
    {
        $uid=(int)$uid;
        $gallery = $db->get_one("SELECT * FROM `{$tablepre}group_gallery` WHERE imgid='$uid' LIMIT 1");
        if(!$gallery) break;
        $g=$db->get_one("SELECT goods_name FROM `{$tablepre}goods_group` WHERE uid='$gallery[goods_id]' LIMIT 1");
        if($g) admin_log("删除团购商品相册：$g[goods_name]");
...
else if($action=='ajax_edit')
{
    ...
    {
        $db->query("UPDATE `{$tablepre}goods_group` SET `$field`='$v' WHERE uid='$uid'");
        $db->free_result();
    }
    exit;
}
...
else if($action=='approval')
{
    ...
    $g=$db->get_one("SELECT goods_name,supplier_id FROM `{$tablepre}goods_group` WHERE uid='$uid' LIMIT 1");
    if(!$g) exit('ERROR:检索不到指定的团购活动');
    $row=array(
        'approval'=>$v
    );
    if($v==-1) $row['reject']=dhtmlchars($reject);
    $db->update("`{$tablepre}goods_group`",$row," uid='$uid' AND approval='0'");
    $db->free_result();
    if($v==-1) add_score($g['supplier_id'],floatval($mm_groupgd_point),'拒绝团购','拒绝:'.$g['goods_name']);
    admin_log($v==-1?"拒绝团购商品：$g[goods_name]":"审核团购商品：$g[goods_name]");
...
function del_goods($uid)
{
    ...
    $rtl = $db->get_one("SELECT uid,goods_file1,goods_name FROM `{$tablepre}goods_group` WHERE uid = '$uid' LIMIT 1");
    if(!$rtl) exit;
    admin_log("删除团购商品：$rtl[goods_name]");
