...
if($action=='list')
{
    ...
    $q = $db->query("SELECT uid,goods_name,goods_file1,goods_sale_price,goods_code,goods_category,goods_stock,supplier_id,type 
    ...
    while ($rtl = $db->fetch_array($q))
    {
        ...
        $rtl['edit'] = "admincp.php?module=$module&action=edit&uid=$rtl[uid]&prev_url=$prev_url";
        $rtl['goods_file1'] = ProcImgPath($rtl['goods_file1']);
        $rtl['preorder']=$rtl['type']==9?'<b class="red">是</b>':'否';
        
        $rtl['goods_url'] = GetBaseUrl('product',$rtl['uid'],'action',$rtl['supplier_id']);
...
else if($action=='edit')
{
    ...
    $product = $db->get_one("SELECT * FROM `{$tablepre}goods_table` WHERE uid='$uid' LIMIT 1");
    ...
    require_once template('goods_add');
    footer();
}
else if ($action=='delimg')    //ajax删除单个相册图片
{
    do
    {
        ...
        if(!$gallery) break;
        $g=$db->get_one("SELECT goods_name FROM `{$tablepre}goods_table` WHERE uid='$gallery[goods_id]' LIMIT 1");
        if($g) admin_log("删除商品相册：$g[goods_name]");
...
else if($action=='bat_down')
{
    do
    {
        ...
        foreach ($uid_check as $key=>$val)
        {
            ...
            $rtl=$db->get_one("SELECT * FROM `{$tablepre}goods_table` WHERE uid='$val' LIMIT 1");
    		if(!$rtl) continue;
    		admin_log("下架商品：$rtl[goods_name]");
    		unset($rtl['goods_hit']);
...
else if($action=='ajax_edit')
{
    ...
    {
        $db->query("UPDATE `{$tablepre}goods_table` SET `$field`='$v' WHERE uid='$uid'");
        $db->free_result();
    }
    exit;
}
...
else if($action=='search_combine')    //搜索商品，用于组合购买
{
    ...
    {
        if($uid<0) break;
        $product=$db->get_one("SELECT supplier_id FROM `{$tablepre}goods_table` WHERE uid='$uid' LIMIT 1");
        if(!$product) break;
        $sup_id=$product['supplier_id'];
        $key=dhtmlchars($_POST['key']);
        $q=$db->query("SELECT uid,goods_file1,goods_name,goods_sale_price 
                       FROM `{$tablepre}goods_table` WHERE goods_name LIKE '%$key%' AND supplier_id='$sup_id' AND type IN (0,1,3,9) AND uid<>'$uid' 
                       LIMIT 10");
        while ($rtl=$db->fetch_array($q))
        {
            $rtl['goods_file1']=ProcImgPath($rtl['goods_file1']);
...
function del_goods($uid)
{
    ...
    $rtl = $db->get_one("SELECT uid,goods_file1,goods_name FROM `{$tablepre}goods_table` WHERE uid = '$uid' LIMIT 1");
    if(!$rtl) exit;
    admin_log("下架商品：$rtl[goods_name]");
