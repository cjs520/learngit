 ...
 * $Date: 2008-02-27 $
 * $Id: showgd.inc.php www.mvmmall.cn$
 * ---------------------------------------------
*/
...
if($action=='list')
{
    ...
    $q = $db->query("SELECT uid,goods_name,goods_file1,goods_sale_price,goods_code,goods_category,goods_stock,supplier_id 
                     FROM `{$tablepre}goods_show` $search_sql $order_sql LIMIT $from_record, $list_num");
    while ($rt = $db->fetch_array($q))
    {
        ...
        $rt['goods_url'] = GetBaseUrl('product',$rt['uid'],'action',$rt['supplier_id']);
    ...
    require_once template('goods_show');
    footer();
}
else if($action=='edit')
{
	$goods_id=$uid=(int)$uid;
    $product = $db->get_one("SELECT * FROM `{$tablepre}goods_show` WHERE uid='$uid' LIMIT 1");
    ...
    if ($_POST && (int)$setp == 1)
    {
        ...
        $db->free_result();
        admin_log("修改展示商品：$goods_name");
        show_msg('success',base64_decode($_POST['prev_url']));
    } 
    
    $mm_brand = $cache->get_cache('brand');
    
    $photo = $db->get_all("SELECT imgid,thumb FROM `{$tablepre}show_gallery` WHERE goods_id='$uid'");//相册数据
    ...
    if($type==1)    //组合购买
    {
        ...
        while ($rtl=$db->fetch_array($q))
        {
            ...
            {
                $g=$db->get_one("SELECT goods_name,goods_file1 FROM `{$tablepre}goods_show` WHERE uid='$rtl[com_uid]' LIMIT 1");
                if(!$g) continue;
    ...
    require_once template('goods_show_add');
    footer();
}
else if ($action=='delimg')    //ajax删除单个相册图片
{
    ...
    {
        $uid=(int)$uid;
        $gallery = $db->get_one("SELECT * FROM `{$tablepre}show_gallery` WHERE imgid='$uid' LIMIT 1");
        if(!$gallery) break;
        $g=$db->get_one("SELECT goods_name FROM `{$tablepre}goods_show` WHERE uid='$gallery[goods_id]' LIMIT 1");
...
else if($action=='ajax_edit')
{
    ...
    {
        $db->query("UPDATE `{$tablepre}goods_show` SET `$field`='$v' WHERE uid='$uid'");
        $db->free_result();
    }
    exit;
}
...
else if($action=='search_combine')    //搜索商品，用于组合购买
{
    ...
    {
        if($uid<0) break;
        $product=$db->get_one("SELECT supplier_id FROM `{$tablepre}goods_show` WHERE uid='$uid' LIMIT 1");
        if(!$product) break;
        $sup_id=$product['supplier_id'];
        $key=dhtmlchars($_POST['key']);
        $q=$db->query("SELECT uid,goods_file1,goods_name,goods_sale_price 
                       FROM `{$tablepre}goods_show` WHERE goods_name LIKE '%$key%' AND supplier_id='$sup_id' 
                       LIMIT 10");
        while ($rtl=$db->fetch_array($q))
        {
...
function del_goods($uid)
{
    ...
    $rtl = $db->get_one("SELECT uid,goods_file1,goods_name FROM `{$tablepre}goods_show` WHERE uid = '$uid' LIMIT 1");
    if(!$rtl) exit;
    admin_log("删除展示商品：$rtl[goods_name]");
